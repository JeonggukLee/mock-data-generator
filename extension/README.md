...
| **フォーマット指定** | `{...}` 内をフォーマット指定または名前付きリスト名として解釈、外側はリテラル。リスト名は **名前付きリスト** で複数定義可能 |
| **数値範囲** | min / max / 小数桁 / 生成モード（ランダム・シーケンス増/減）/ ステップ |
| **日付範囲** | min / max / モード / ステップ / 単位（日・月・年）<br>※月単位の日末は次月最終日に clamp（`2026-01-31` + 1ヶ月 → `2026-02-28`）<br>※min/max が空または不正な日付の場合は「日付範囲ルール: min/max が不正な日付形式です」のエラーで生成停止 |
...
### 名前付きリスト（フォーマット指定ルール内）

フォーマット指定ルールでは、ルールごとに **複数の名前付きリスト** を定義し、パターン内 `{Name}` で参照できます。`{Name}` の中身がリスト名と完全一致したらリストから 1 件抽選、未マッチなら従来通り `A`/`9` 等のフォーマット文字として解釈されます。

| 項目 | 説明 |
| --- | --- |
| 定義 | UI のフォーマット指定ルール内「+ リスト追加」で名前と値を追加 |
| 値の入力 | カンマ区切り。値内コンマは `\,`、リテラル `\` は `\\` でエスケープ（値リストと同じ） |
| 参照 | パターン内で `{<名前>}`（リスト名は中括弧の中に他文字を入れず単独で記述） |
| 複数参照 | 同名リストを複数回参照すると **毎回独立抽選** される |
| 併用 | 同じパターン内で フォーマット文字 / リテラル / リスト参照 を自由に混在可能 |

**例（リスト `L1=[東京,大阪,名古屋]` / `L2=[A,B]` を定義した場合）:**

- `{L1}` → `東京` または `大阪` または `名古屋`
- `{L1}-{L2}-{9999}` → `大阪-B-4217`
- `USER_{L1}-{9}{9}` → `USER_名古屋-42`
- `{L1}-{L1}` → `東京-大阪`（同名でも毎回独立抽選）
