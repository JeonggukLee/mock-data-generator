...
- **フォーマット指定**: `{...}` 内のみフォーマット指定として解釈、外側は一般リテラル
  - 文字種: `A`=英大 / `a`=英小 / `9`=数字 / `X`=英数 / `H`=ひらがな / `K`=カタカナ / `S`=記号 / `J`=漢字（常用）
...
- **生成しない (NULL)**: 常に NULL を出力（カラムは INSERT に含む）。NOT NULL カラムには警告が出る
- **固定文**: 毎行同じ固定文字列を出力（SQL では文字列リテラル）
- **別カラム値参照**: 同一行の別カラム値を流用。`比較` で同値 / より大きい / より小さい を選択。大きい/小さい時は `オフセット最小`〜`オフセット最大` で範囲を指定（参照先型に応じた単位: 数値 / 日 / 月 / 年 / 秒 / 分 / 時）。参照先カラムは本カラムより前に定義されている必要がある
